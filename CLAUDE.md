...
After making changes, always check whether any patch files in `/projects/monarck/infra/patches/` reference the modified files. Those patches apply to compiled ClawPort chunks and OpenClaw internals -- if a patched file is regenerated by a build, the patch may need updating too.

When changing dashboard source that is deployed through Monarck infra, also update the matching Ansible overlay files in `/Users/pinchy/projects/monarck/infra/ansible/roles/clawport/files/patches/` and the copy list in `/Users/pinchy/projects/monarck/infra/ansible/roles/clawport/tasks/main.yaml` if new files need to be overlaid. A dashboard-only code change is not enough when `clawport-push` is the deployment path.

If a Monarck Ansible deploy or ClawPort push breaks mid-role, always provide the resume command with `--start-at-task` in your response so the next run can pick up from the failed task instead of rerunning the entire play. For ClawPort patch-script failures, prefer restarting at `Copy ClawPort ConversationView patch script` so the updated local script is recopied before the patch task reruns.
